...
> 2. This module is **Progress Photos**. Your DB table is `progress_photos`
>    (already defined in `../../supabase-schema.sql`).
> 3. Best reference to copy: **drawing-register (it uses the same file-upload pattern you need for photos)**.
> 4. Work only inside this folder, on branch `module/progress-photos`, then PR to `main`.
