# Module: productivity-rates

> **Claude / developer: read this first.**
> 1. Read `../../MODULE_CONTRACT.md` and `../../CONTRIBUTING.md` (NOT auto-loaded).
> 2. This module is **Productivity Rates** (Phase 2). Your DB table is `productivity_rates`
>    (defined in `../../supabase-schema.sql`; starter columns only — extend as needed).
> 3. Best reference to copy: **risk-register (plain CRUD with rate calculations)**.
> 4. Work only inside this folder, on branch `module/productivity-rates`, then PR to `main`.
