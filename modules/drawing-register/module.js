...
window.DrawingRegister = (function () {
  ...
  function periodChartSVG(data) {
    ...
    var bars = data.map(function (d,i){
      ...
      return '<rect x="'+(X(i)-bw/2)+'" y="'+(padT+innerH-bh)+'" width="'+bw+'" height="'+bh+'" fill="var(--pd-line,#DCDBDB)" rx="3"><title>'+
    ...
    function dots(key,color){ return data.map(function (d,i){
      return '<circle cx="'+X(i)+'" cy="'+Y(d[key])+'" r="2.75" fill="'+color+'"><title>'+Fmt.esc(d.label)+': '+d[key]+'</title></circle>';
    }).join(''); }
    ...
    var xlabels = data.map(function (d,i){
      ...
      return '<text x="'+X(i)+'" y="'+(h-8)+'" font-size="9.5" text-anchor="middle" fill="currentColor" opacity="0.6">'+Fmt.esc(d.label)+'</text>';
    }).join('');
